    ...
    while True:
        ...
        if len(faceRects) > 0:
            for faceRect in faceRects:
                ...
                # 参数1：字体文件路径，参数2：字体大小
                draw.text((x + 25, y - 95), 'yao_li:{:.2%}'.format(face_probe[0]), (255, 0, 0), font=font)
                draw.text((x + 25, y - 70), 'yao_ti:{:.2%}'.format(face_probe[1]), (255, 0, 0), font=font)
                w = cv2.waitKey(2)
                # 如果输入q则退出循环
                if w & 0xFF == ord('w'):
                    ...
                    if np.max(a)>0.9:
                        ...
                        if user[0][0]==1:
                            draw.text((x + 25, y - 40), '你在疲劳驾驶', (255, 0, 0), font=font)
                            cv2.waitKey(1000)
